...
SS <- log(long_data$sample_size_rep) # log-transform reported sample size
SS <- (SS - mean(SS))/(sd(SS)*2) # standardize reported sample size
BM <- (long_data$body_mass - mean(long_data$body_mass))/(sd(long_data$body_mass)*2) # standardize body mass
SL <- ifelse(long_data$same_lab=="Yes",1,0) # create an indicator for whether longevity and sample size both come from the same lab/source or not
...
## estimate the direct effect of sociality
## to estimate the direct effect, we need to control for sample size as a mediating variable and indicator for different sources as well
...
mean(MLrc_te_cont) # 0.52
PI(MLrc_te_cont) # -0.93, 1.97
...
for(j in 1:n_spp) {
  for(i in 1:n_obs) {
    ...
    MLrc_de_SC2[i,] <- rmvnorm(1,rep(MLrc_de_post$a_ML[i,2] + MLrc_de_post$bBM_ML[i,2]*BM[i,j] + MLrc_de_post$bSS_ML[i,2]*SS[i,j],n_spp),diag(MLrc_de_post$sigma_ML[i],n_spp,n_spp)%*%R) # social 
  }
}

## compute the contrast by subtracting solitary from social species
MLrc_de_cont <- MLrc_de_SC2 - MLrc_de_SC1
mean(MLrc_de_cont) # 0.37
PI(MLrc_de_cont) # -0.92, 1.69
...
mean(MLrs_te_cont) # 0.56
PI(MLrs_te_cont) # -0.83, 1.93
...
mean(MLrs_de_cont) # 0.40
PI(MLrs_de_cont) # -0.77, 1.62


## Figure 2
## compute density for each effect and contrast
