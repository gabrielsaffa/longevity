...
ML_rc <- (long_data$ML_rc - mean(long_data$ML_rc)) / (sd(long_data$ML_rc)*2) # standardize longevity
...
saveRDS(MLrc_te,"MLrc_te.RDS") # save the fitted model
...
saveRDS(MLrc_de,"MLrc_de.RDS") # save the fitted model
...
saveRDS(MLrs_te,"MLrs_te.RDS") # save the fitted model
...
saveRDS(MLrs_de,"MLrs_de.RDS") # save the fitted model


## read the models and extract posterior samples
MLrc_te <- readRDS("MLrc_te.RDS")
MLrc_de <- readRDS("MLrc_de.RDS")
MLrs_te <- readRDS("MLrs_te.RDS")
MLrs_de <- readRDS("MLrs_de.RDS")
...
mean(MLrc_te_cont) # 0.50
PI(MLrc_te_cont) # -0.97, 1.98
...
mean(MLrc_de_cont) # 0.44
PI(MLrc_de_cont) # -0.90, 1.77
...
mean(MLrs_te_cont) # 0.51
PI(MLrs_te_cont) # -0.86, 1.88
...
mean(MLrs_de_cont) # 0.44
PI(MLrs_de_cont) # -0.89, 1.78
